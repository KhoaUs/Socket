...
void processEmail(SOCKET clientSocket, const string uid_Email) {
    ...
    size_t readReceiptPos = emailData.find("Read-Receipt-To: " + DEFAULT_EMAIL);
    if (readReceiptPos != string::npos) {
        // Send a read receipt acknowledgment
        const char* readReceiptAck = "X-Read-Receipt: Received\r\n";
        send(clientSocket, readReceiptAck, strlen(readReceiptAck), 0);
    }
...
//tạo header
    string emailHeaders = "Read-Receipt-To: " + DEFAULT_EMAIL + "\r\n";
    emailHeaders += "Disposition-Notification-To: " + DEFAULT_EMAIL + "\r\n";
    emailHeaders += "X-Confirm-Reading-To: " + DEFAULT_EMAIL + "\r\n";

    // Include the headers in the email content
    string emailContent = emailHeaders + receiveData(clientSocket);
    // Khởi tạo Winsock
    WSADATA wsaData;
